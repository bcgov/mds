...
class NonComplianceLegislation(Base):
    ...
    __table_args__ = {
        'comment': 'Contains the additional details about the contravention(s) relating to an issued order; i.e. estimated incident date, noncompliance description, regulations/act contravened.'}
    ...
    parent_act_id = db.Column(db.Integer, db.ForeignKey(
        'legislation_act.legislation_act_id'))
    ...
    regulation_legislation_act = db.relationship("LegislationAct",
                                                 ...
                                                 lazy='joined')
    regulation_legislation_act_section = db.relationship(
        "LegislationActSection", lazy='joined')
    compliance_article = db.relationship(
        "LegislationComplianceArticle", lazy='joined')

    parent_act = association_proxy('parent_legislation_act', 'act')
    regulation_act = association_proxy('regulation_legislation_act', 'act')
    section = association_proxy(
        'regulation_legislation_act_section', 'section')
    compliance_article_comments = association_proxy(
        'compliance_article', 'comments')
