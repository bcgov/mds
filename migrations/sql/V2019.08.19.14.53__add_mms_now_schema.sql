...
BEGIN
    IF EXISTS(select from pg_foreign_server where srvname = 'oracle_fdw') THEN
        IF NOT EXISTS(SELECT FROM information_schema.schemata WHERE schema_name = 'mms_now_nros') THEN
        ...
        IF NOT EXISTS(SELECT FROM information_schema.schemata WHERE schema_name = 'mms_now_vfcbc') THEN
