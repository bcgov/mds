...
BEGIN
   EXECUTE format('
   DROP SEQUENCE IF EXISTS public.permit_conditions_standard_permit_condition_id_seq CASCADE;
   ...
   NO MAXVALUE
   CACHE 1;'
 , (SELECT COALESCE ((SELECT MAX(standard_permit_condition_id) FROM standard_permit_conditions),0)));
