...
DO $$                  
    BEGIN 
		IF (SELECT to_regclass('public.permit_amendment') is null) THEN 
			...
			insert into permit (mine_guid, permit_no, permit_status_code, create_user, update_user)
			...
			(select permit_guid from permit where permit_no = (select permit_no from temp_old_permit where mine_party_appt.permit_guid=temp_old_permit.permit_guid) and mine_guid=mine_party_appt.mine_guid limit 1)
			...
			ALTER TABLE ONLY permit_amendment ADD CONSTRAINT permit_amendment_permit_fkey FOREIGN KEY (permit_id) REFERENCES permit(permit_id);
			ALTER TABLE ONLY permit_amendment ADD CONSTRAINT permit_amendment_type_code_fkey FOREIGN KEY (permit_amendment_type_code) REFERENCES permit_amendment_type_code(permit_amendment_type_code);
