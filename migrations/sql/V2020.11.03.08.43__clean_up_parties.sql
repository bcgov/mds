...
SELECT party_guid into temporary table parties_to_delete FROM party WHERE party_guid not IN (SELECT party_guid FROM parties_in_use where party_guid is not NULL);
