...
BEGIN
    ...
    WHERE table_name='etl_permit' and column_name='permit_amendment_guid'
	        ...
	        THEN
    ALTER TABLE
    IF EXISTS ETL_PERMIT RENAME COLUMN permit_guid TO permit_amendment_guid;
ALTER TABLE
IF EXISTS ETL_PERMIT
ADD COLUMN permit_guid uuid;
END
IF ;
	       
			update etl_permit set permit_guid = 
			(select p.permit_guid
from permit p join permit_amendment pa on p.permit_id=pa.permit_id
where etl_permit.permit_amendment_guid=pa.permit_amendment_guid limit 1
)
