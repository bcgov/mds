...
(254)        ,
        effective_date      date        ,
        person_combo_id     varchar
(600),
        mgr_combo_id        varchar
(600)
    );
SELECT count(*)
FROM ETL_MANAGER
...
WITH
    ...
    (
        SELECT
            SUBSTRING(now_contact.cid from 1 for 7
    ) AS mine_no      ,
        ...
        WHERE
            SUBSTRING
(now_contact.type_ind from 3 for 1) = 'Y'
    ),
...
::numeric phone_no                        , --Extract numbers only from phone_no field
            COALESCE
(NULLIF
(regexp_replace
(contact_info.email,' ', '', 'g'),''),'Unknown') email    ,
            COALESCE
(contact_info.add_dt,now
()) effective_date
...
(
    ...
    CASE
                ...
                ELSE
                    SUBSTRING(phone_no from 1
for 3)||'-'||
                    SUBSTRING
(phone_no from 4 for 3)||'-'||
                    SUBSTRING
(phone_no from 7 for 4)
END AS phone_no ,
            email           ,
            effective_date
    ...
    (
        SELECT *
    FROM existing_person_info_format_phone
UNION
    SELECT
        party_guid      ,
        person_combo_id ,
...
END AS phone_no ,
    ...
    (
        SELECT *
    FROM new_person_info_format_phone
UNION
    SELECT
        party_guid      ,
        person_combo_id ,
