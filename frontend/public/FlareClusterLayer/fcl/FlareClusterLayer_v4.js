...
define([
  ...
  var FlareClusterLayer = /** @class */ (function(_super) {
    ...
    function FlareClusterLayer(options) {
      ...
      if (options.clusterAreaDisplay) {
        _this.clusterAreaDisplay =
          options.clusterAreaDisplay === "none" ? undefined : options.clusterAreaDisplay; // when to display the area (convex hull) of the points for each each cluster
      }
      ...
      for (var i = 0; i < dataLength; i++) {
        ...
        if (this._isClustered) {
          ...
          for (var j = 0, jLen = this._gridClusters.length; j < jLen; j++) {
            ...
            cl.x =
              cl.clusterCount > 0 ? (xVal + cl.x * cl.clusterCount) / (cl.clusterCount + 1) : xVal;
            cl.y =
              cl.clusterCount > 0 ? (yVal + cl.y * cl.clusterCount) / (cl.clusterCount + 1) : yVal;
    ...
    FlareClusterLayer.prototype._createCluster = function(gridCluster) {
      ...
      cluster.clusterGraphic.symbol = this.clusterRenderer.getClassBreakInfo(
        cluster.clusterGraphic
      ).symbol;
    ...
    FlareClusterLayer.prototype._createSurface = function() {
      ...
      if (this._is2d) {
        ...
        surfaceParentElement =
          this._activeView.canvas.parentElement || this._activeView.canvas.parentNode;
      }
    ...
    FlareClusterLayer.prototype._initCluster = function() {
      ...
      var clonedClusterElement = this._createClonedElementFromGraphic(
        this._activeCluster.clusterGraphic,
        this._activeCluster.clusterGroup
      );
      this._addClassToElement(clonedClusterElement, "cluster");
      // create the cluster text shape
      var clonedTextElement = this._createClonedElementFromGraphic(
    ...
    FlareClusterLayer.prototype._initFlares = function() {
      ...
      var subTypeFlares =
        !singleFlares && (gridCluster.subTypeCounts && gridCluster.subTypeCounts.length > 0);
      ...
      var clusterScreenPoint = this._activeView.toScreen(
      ...
      var _loop_1 = function(i_2, len_1) {
        ...
        if (f.textGraphic) {
          var flareTextElement = this_1._createClonedElementFromGraphic(
            f.textGraphic,
            f.flareGroup
          );
    ...
    FlareClusterLayer.prototype._createTooltip = function(flare) {
      ...
      for (var i = 0, len = lines.length; i < len; i++) {
        var textShape = tooltipGroup
          .createText({ x: xPos, y: yPos + i * 10, text: lines[i], align: "middle" })
    ...
    FlareClusterLayer = __decorate(
      [asd.subclass("FlareClusterLayer"), __metadata("design:paramtypes", [Object])],
      FlareClusterLayer
    );
