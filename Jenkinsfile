            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'github-account', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        sh("git checkout newMaster")
                        sh("git merge --squash origin/newpr3")
                        sh("git commit -m 'Merge develop into master'")
                        sh('git push origin newMaster https://${GIT_USERNAME}:${GIT_PASSWORD}@<REPO> ')
                    }
                }
            }
        }
