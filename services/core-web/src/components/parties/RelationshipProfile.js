...
const propTypes = {
  ...
  mine_permits: PropTypes.arrayOf(CustomPropTypes.permits).isRequired,
};
...
export class RelationshipProfile extends Component {
  ...
  updateState() {
    ...
    if (mine && Object.keys(this.state.permitsMapping).length < this.props.mine_permits.length) {
      this.setState({ permitsMapping: mapPermitGuidToNumber(this.props.mine_permits) });
    }
