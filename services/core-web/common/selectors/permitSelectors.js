...
export const { getPermits, getDraftPermits, getPermitConditions, getEditingConditionFlag } = permitReducer;
...
export const getDraftPermitAmendmentForNOW = createSelector(
  ...
  (draftPermits, noticeOfWork) => {
    ...
    return draftPermit && draftPermit.permit_amendments.length > 0 ? draftPermit.permit_amendments.filter(
      (amendment) =>
