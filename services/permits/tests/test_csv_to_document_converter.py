...
def test_csv_to_document_success(csv_converter, mock_csv_file):
    documents = csv_converter.run(mock_csv_file)
    
    assert len(documents) == 1
    assert documents[0]['text'] == 'test condition'
...
def test_csv_to_document_empty_meta_fields(tmp_path):
    ...
    documents = CSVToDocument().run(csv_file)
...
def test_csv_to_document_invalid_csv(tmp_path):
    ...
    assert "Error processing CSV file" in str(exc.value)
...
def test_csv_to_document_multiple_rows(tmp_path):
    ...
    assert documents[0]['text'] == 'first condition'
    assert documents[1]['text'] == 'second condition'