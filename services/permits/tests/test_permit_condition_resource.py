...
def mock_run_permit_condition_pipeline(mocker):
    ...
    return mocker.patch.object(run_permit_condition_pipeline, 'delay', return_value=pending_task)

@pytest.fixture
def mock_store_temporary(mocker):
    mocked_file = mock.MagicMock(name='abc.123.pdf', close=mock.MagicMock())
    return mocker.patch('app.permit_conditions.resources.permit_condition_resource.store_temporary', return_value=mocked_file)
...
@pytest.mark.asyncio
async def test_extract_permit_conditions_success(mock_store_temporary, mock_run_permit_condition_pipeline):
...
async def test_extract_permit_conditions_invalid_file_type():
    ...
    assert response.json()['detail'] == 'Invalid file type. Only PDF files are supported.'
...
def test_results_incomplete(mock_async_result):
    ...
    assert response.json()['detail'] == ("Task has not completed yet. Current status: PENDING")
