...
patch.dict(os.environ, {"AZURE_OPENAI_ENDPOINT": "http://test.test", "AZURE_OPENAI_API_KEY": "test"}).start()


def test_run_with_valid_data():
    data = ChatData(messages=[ChatMessage.from_user("test_message")], documents=[])
    generation_kwargs = {}
    expected_reply = ChatMessage(content="Mocked reply", role='assistant', name=None, meta={'usage': {'completion_tokens': 10, 'prompt_tokens': 5, 'total_tokens': 15}, 'finish_reason': 'stop'})

    with patch.object(CachedAzureOpenAIChatGenerator, 'fetch_result', return_value=expected_reply):
...
def test_run_with_valid_data_multiple_iterations():
    ...
    # another continuation request to complete the responsew
    expected_reply = ChatMessage(content="Mocked reply", role='assistant', name=None, meta={'usage': {'completion_tokens': 10, 'prompt_tokens': 5, 'total_tokens': 15}, 'finish_reason': 'length'})
    expected_reply2 = ChatMessage(content="reply continued", role='assistant', name=None, meta={'usage': {'completion_tokens': 1, 'prompt_tokens': 2, 'total_tokens': 3}, 'finish_reason': 'stop'})

    with patch.object(CachedAzureOpenAIChatGenerator, 'fetch_result', side_effect=[expected_reply, expected_reply2]) as mock_fetch_result:
        generator = CachedAzureOpenAIChatGenerator()
        ...
        mock_fetch_result.assert_called_with(data.messages + [expected_reply, ChatMessage.from_user("Continue!")], {})
