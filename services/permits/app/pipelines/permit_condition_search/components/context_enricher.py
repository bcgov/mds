...
class ContextEnricher:
    ...
    def _format_context_chain(self, parents: List[Document], siblings: List[Document], children: List[Document]) -> Dict[str, dict]:
        """
    ...
    def _collect_all_related_ids(self, documents: List[Document], max_levels: Optional[int]) -> Tuple[Set[str], Set[str], Set[str]]:
        ...
        for doc in documents:
            ...
            if doc_sibling_ids:
                ...
                selected_siblings = doc_sibling_ids[max(0, mid-2):mid] + doc_sibling_ids[mid:mid+2]
    ...
    def run(self, documents: List[Document], max_levels: Optional[int] = None):
        ...
        parent_ids, sibling_ids, child_ids = self._collect_all_related_ids(documents, max_levels)
        ...
        for doc in documents:
            ...
            if parent_ids := doc.meta.get("parent_ids", []):
                ...
                parent_chain = [related_docs[pid] for pid in parent_ids if pid in related_docs]

            # Get siblings
            if sibling_ids := doc.meta.get("sibling_ids", []):
                mid = len(sibling_ids) // 2
                selected_siblings = sibling_ids[max(0, mid-2):mid] + sibling_ids[mid:mid+2]
                siblings = [related_docs[sid] for sid in selected_siblings if sid in related_docs]

            # Get children
            if child_ids := doc.meta.get("child_ids", []):
                children = [related_docs[cid] for cid in child_ids[:2] if cid in related_docs]
