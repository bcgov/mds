...
patterns = [
    {
        ...
        "pattern": re.compile(
            r"^(?:\(|)([A-Z]{0,3}+)(?:\)|)[\.\)\s]*\s+"
        ),  # Uppercase letter(s) optionally surrounded by parentheses, followed by optional separator or space
    },
    {
        "name": "number",
        "pattern": re.compile(
            r"^(?:\(|)(\d+)(?:\)|)[\.\)\s]*\s+"
        ),  # Number optionally surrounded by parentheses, followed by optional separator or space
    },
    {
        "name": "lowercase_letter",
        "pattern": re.compile(
            r"^(?:\(|)([a-z]{0,3}+)(?:\)|)[\.\)\s]*\s+"
        ),  # Lowercase letter(s) optionally surrounded by parentheses, followed by optional separator or space
    },
    {
        "name": "roman_numeral",
        "pattern": re.compile(
            r"^(?:\(|)([ivx]+)(?:\)|)\s*"
        ),  # Roman numeral optionally surrounded by parentheses (assuming this is always followed by a space)
    },
]
...
def split_numbering(paragraphs):
    ...
    for idx, paragraph in enumerate(paragraphs):
        ...
        if paragraph["regex"] == "lowercase_letter" and _is_actually_roman_numeral(
            paragraphs, paragraph, idx
        ):
...
def build_hierarchy(paragraphs):
    ...
    for idx, paragraph in enumerate(paragraphs):
        ...
        if regx:
            ...
            level, skip_last_update = _update_level(
                paragraphs, structure, last_paragraph, level, idx, paragraph, regx
            )

            # Update the hierarchy state based on the level we're at (e.g. {section: A, paragraph: 1, subparagraph: 2})
            hierarchy_state = _update_hierarchy_state(
                hierarchy_state, level, paragraph["numbering"]
            )
...
def _is_actually_roman_numeral(paragraphs, paragraph, idx):
    ...
    regx = paragraph["regex"]
    prev_p = next(
        (p for p in reversed(paragraphs[:idx]) if p["numbering"] is not None), None
    )
...
def _percent_increase(prev, curr):
    ...
    if prev_bounding_box.get("left") and bounding_box.get("left"):
        increase = (
            (bounding_box["left"] - prev_bounding_box["left"])
            * 100
            / prev_bounding_box["left"]
        )
        return increase
