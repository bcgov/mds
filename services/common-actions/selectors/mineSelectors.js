...
export const getMineBasicInfoList = state =>
  mineReducer.getMineBasicInfoList(state);
export const getMineDocuments = state => mineReducer.getMineDocuments(state);
export const getSubscribedMines = state =>
  mineReducer.getSubscribedMines(state);

export const getIsUserSubscribed = createSelector(
  [getSubscribedMines, getMineGuid],
  (subscribedMines, mineGuid) =>
    mineGuid
      ? subscribedMines.map(({ mine_guid }) => mine_guid).includes(mineGuid)
      : false
);
