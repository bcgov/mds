...
#GET
def test_get_ministry_contact_not_found(test_client, db_session, auth_headers):
    fake_guid = uuid.uuid4()
    get_resp = test_client.get(
        f'/ministry-contacts/{fake_guid}', headers=auth_headers['full_auth_header'])
...
#PUT
def test_put_ministry_contact_not_found(test_client, db_session, auth_headers):
    data = {"first_name": "First", "last_name": "Last", "phone_number": "111-111-1111"}
    fake_guid = uuid.uuid4()

    put_resp = test_client.put(
        f'/ministry-contacts/{fake_guid}', json=data, headers=auth_headers['full_auth_header'])
    put_data = json.loads(put_resp.data.decode())
    assert put_resp.status_code == 404
    assert 'not found' in put_data['message']


def test_put_ministry_contact_success(test_client, db_session, auth_headers):
    contact = MinistryContactFactory()
