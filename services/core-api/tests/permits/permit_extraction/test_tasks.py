...
def test_initialize_single_permit_extraction_success(permit_search_service_mock, permit_amendment_document_mock, 
    ...
    permit_amendment_document_mock.find_by_permit_amendment_document_name.return_value = document_mock
    ...
    with mock.patch('app.api.mines.permits.permit_extraction.tasks.poll_update_permit_extraction_status') as poll_mock:
        ...
        result = initialize_single_permit_extraction('test_document', 'test_amendment_guid')
        
        assert result == task_mock.permit_extraction_task_id
        permit_amendment_document_mock.find_by_permit_amendment_document_name.assert_called_with('test_document')
...
def test_initialize_single_permit_extraction_invalid_document(permit_amendment_document_mock, celery_app, test_client, db_session):
    permit_amendment_document_mock.find_by_permit_amendment_document_name.return_value = None
    
    with pytest.raises(ValueError, match="Invalid document test_document"):
        initialize_single_permit_extraction('test_document', 'test_amendment_guid')
