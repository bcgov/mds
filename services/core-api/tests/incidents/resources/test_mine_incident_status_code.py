...
class TestGetIncidentStatusCodeResource:
    ...
    def test_get_incident_status_codes(self, test_client, db_session, auth_headers):
        ...
        get_resp = test_client.get(f'/incidents/status-codes', headers=auth_headers['full_auth_header'])
        get_data = json.loads(get_resp.data.decode())
        assert get_resp.status_code == 200
        assert len(MineIncidentStatusCode.active()) > 0
        assert len(get_data['records']) == len(MineIncidentStatusCode.active())
