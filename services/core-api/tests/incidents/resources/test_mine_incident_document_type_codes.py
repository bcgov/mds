...
class TestGetIncidentDocumentTypeCodeResource:
    ...
    def test_get_incident_determination_types(self, test_client, db_session, auth_headers):
        ...
        get_resp = test_client.get(f'/incidents/document-types', headers=auth_headers['full_auth_header'])
        get_data = json.loads(get_resp.data.decode())
        assert get_resp.status_code == 200
        assert len(MineIncidentDocumentTypeCode.active()) > 0
        assert len(get_data['records']) == len(MineIncidentDocumentTypeCode.active())
