...
def test_get_compliance_articles_with_one_or_more_query_parameters(test_client, auth_headers, db_session):
    ...
    assert get_resp.status_code == 200, get_resp.response
    get_data = json.loads(get_resp.data.decode())
    assert len(get_data['records']) == ComplianceArticle.query.filter_by(article_act_code='HSRCM',
                                                                         expiry_date='9999-12-31').count()
