...
class TestApplicationProponentResource:
    ...
    def test_get_proponent_now_application_failure(self, test_client, db_session, auth_headers):
        """Should return error if now_application_guid doesn't exist"""
        now_application_guid = "3e2adc87-2312-40dc-9942-327ea7e2fff8"
        get_resp = test_client.get(f'/now-applications/{now_application_guid}/proponent', headers=auth_headers['full_auth_header'])
