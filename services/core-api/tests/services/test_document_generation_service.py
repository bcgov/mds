...
def test_missing_template_returns_fail(test_client, db_session, auth_headers):
    with current_app.test_request_context():
        document_template = DocumentTemplate.query.get('NRL')
        template_data = {'mine_no': '1234456'}
        file_download_resp = DocumentGeneratorService.generate_document(
            document_template, template_data)
