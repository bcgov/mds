...
def register_commands(app):
    ...
    def _create_cypress_data():
        ...
        with app.app_context():
            ...
            minespace_user = MinespaceUserFactory(bceid_username='cypress@bceid')
    ...
    def revoke_mines_act_permits_for_permit(credential_exchange_id, permit_guid):
        from app import auth
        from app.api.verifiable_credentials.anoncred_manager import (
    ...
    def cleanup_untp_map_data_failures(live: bool = False):
        ...
        with current_app.app_context():
            ...
            result = UNTPCredentialManager.delete_any_unsuccessful_untp_push(live)
    ...
    def _seed_user_data(user_name, is_idir):
        ...
        with app.app_context():
            major_mines = db.session.query(Mine).filter_by(major_mine_ind=True).order_by(
                Mine.mine_name).limit(major_mine_count)
            regional_mines = db.session.query(Mine).filter_by(major_mine_ind=False).order_by(
                Mine.mine_name.desc()).limit(regional_mine_count)
            ...
            if is_idir:
                ...
                for mine in all_mines:
                    if mine.mine_guid in subscribed_mine_guids:
                        print(
                            f'Skipping already subscribed mine. Username: {full_user_name}, Mine: {mine.mine_name}'
                        )
                    else:
                        print(
                            f'Creating mine subscription. Username: {full_user_name}, Mine: {mine.mine_name}'
                        )
                        subscription = Subscription(
                            mine_guid=mine.mine_guid, user_name=full_user_name)
                ...
                if not minespace_user:
                    ...
                    from app.api.users.minespace.models.minespace_user_mine import (
                        MinespaceUserMine, )
                    existing_subscriptions = MinespaceUserMine.query.filter_by(
                        user_id=minespace_user.user_id).all()
                    subscribed_mine_guids = [s.mine_guid for s in existing_subscriptions]
                for mine in all_mines:
                    if mine.mine_guid in subscribed_mine_guids:
                        print(
    ...
    def do_prepare_permit_data(csv_path, token):
        ...
        with current_app.app_context():
            if Config.ENVIRONMENT_NAME not in ['local', 'dev', 'test']:
                click.echo(
                    "This command is only available in local, dev and test environments.", err=True)
                return
            ...
            with open(csv_path, 'r') as file:
                ...
                for row in reader:
                    try:
                        ...
                        initialize_single_permit_extraction.delay(document_manager_guid,
    ...
    @click.option(
        ...
        default=False,
        help=
        "If provided, uploads to blob storage to trigger indexing. Otherwise saves to local CSV.")
    @click.option(
        '--output-dir',
        type=click.Path(exists=True, file_okay=False, dir_okay=True),
        required=False,
        help="Directory to save the CSV file in dry-run mode.")
    def bulk_export_and_index_permit_conditions(permit_type, amendment_guids, batch_size, apply,
                                                output_dir):
        """
        ...
        if amendment_guids:
            ...
            permit_amendment_guids = PermitAmendment.find_all_guids_with_extracted_conditions(
                is_now)
            print(
                f"Exporting {len(permit_amendment_guids)} {'Notice of Work' if is_now else 'Major Mine'} permit amendments"
            )
        else:
            permit_amendment_guids = PermitAmendment.find_all_guids_with_extracted_conditions()
            print(
                f"Exporting {len(permit_amendment_guids)} permit amendments with extracted conditions for all types"
            )
