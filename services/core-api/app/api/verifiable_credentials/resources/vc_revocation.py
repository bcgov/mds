...
class VerifiableCredentialRevocationResource(Resource, UserMixin):
    ...
    parser.add_argument(
        ...
        store_missing=False)
    parser.add_argument(
        'comment', type=str, help='GUID of the party.', location='json', store_missing=False)

    @api.expect(parser)
    @api.doc(description='Revokes a verifiable credential by party_guid and credential_exchange_id')
    @requires_any_of([EDIT_PARTY])
    def post(self, party_guid: str):
        if not is_feature_enabled(Feature.VC_ANONCREDS_CORE):
        ...
        if credential_exchange.cred_exch_state not in ["credential_acked", "done"]:
            raise VerificableCredentialWorkflowError(
                f"credential_exchange_id {credential_exchange_id} is not in a state where it can be revoked"
            )

        traction_svc = TractionService()
        revoke_resp = traction_svc.revoke_credential(
            party.active_digital_wallet_connection.connection_id, credential_exchange.rev_reg_id,
