...
class PartyVerifiableCredentialMinesActPermit(AuditMixin, Base):
    ...
    permit_amendment = db.relationship(
        'PermitAmendment', lazy='select', uselist=False, overlaps='vc_credential_exch')
    ...
    @classmethod
    def find_by_cred_exch_id(cls,
                             cred_exch_id,
                             unsafe: bool = False) -> "PartyVerifiableCredentialMinesActPermit":
    ...
    @classmethod
    def find_by_permit_amendment_guid(
            cls, permit_amendment_guid) -> List["PartyVerifiableCredentialMinesActPermit"]:
        return cls.query.filter_by(permit_amendment_guid=permit_amendment_guid).all()

    @classmethod
    def find_issued_by_permit_amendment_guid(
            cls, permit_amendment_guid) -> List["PartyVerifiableCredentialMinesActPermit"]:
        #https://github.com/hyperledger/aries-rfcs/blob/main/features/0036-issue-credential/README.md#states-for-issuer
        return cls.query.filter_by(permit_amendment_guid=permit_amendment_guid).filter(
            PartyVerifiableCredentialMinesActPermit.cred_exch_state.in_(
                cls._active_credential_states)).all()

    @classmethod
    def find_by_permit_guid_and_mine_guid(
            cls, permit_guid, mine_guid) -> List["PartyVerifiableCredentialMinesActPermit"]:
        return cls.query.join(PermitAmendment).join(Permit).filter(
            PermitAmendment.permit_guid == Permit.permit_guid).filter(
                PermitAmendment.mine_guid == mine_guid).all()
