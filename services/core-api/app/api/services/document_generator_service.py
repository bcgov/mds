...
class DocumentGeneratorService():
    ...
    def generate_document(cls, document_template, template_data):
        ...
        # Send the template file to the Document Generator if it doesn't exist
        file_sha = sha256_checksum(fileobj)
        template_exists = cls._check_remote_template(file_sha)
    ...
    def _push_template(cls, document_template, template_file):
        file_name = document_template.file_name
        files = {'template': (file_name, template_file.read(), mimetypes.guess_type(file_name))}
    ...
    @classmethod
    def _check_remote_template(cls, file_sha):
