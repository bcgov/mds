...
class DocumentGeneratorService():
    ...
    def generate_document(cls, document_template, template_data):
        ...
        document_name_extra = template_data.get('document_name_extra')
        document_name_extra = f' {document_name_extra}' if document_name_extra else ''
        date_string = datetime.date.today().strftime("%Y-%m-%d")
        draft_string = ' DRAFT' if template_data.get('is_draft') == True else ''
        document_name = f'{document_template.template_name_no_extension}{draft_string}{document_name_extra} {date_string}.pdf'
