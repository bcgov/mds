...
class StandardPermitConditions(AuditMixin, Base):
    ...
    standard_permit_condition_guid = db.Column(UUID(as_uuid=True),
    ...
    parent_standard_permit_condition_id = db.Column(
        db.Integer,
        db.ForeignKey(
            'standard_permit_conditions.standard_permit_condition_id'))
    ...
    def __repr__(self):
        return '<StandardPermitConditions %r, %r>' % (
            self.standard_permit_condition_id,
