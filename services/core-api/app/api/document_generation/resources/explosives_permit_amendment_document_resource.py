...
class ExplosivesPermitAmendmentDocumentResource(Resource, UserMixin):
    ...
    def generate_explosives_permit_document(cls,
        ...
        explosives_permit_amendment_document_type = ExplosivesPermitDocumentType.query.unbound_unsafe().get(
            document_type_code)

        # Generate the document using the template and template data
        explosives_permit_amendment_guid = token_data['explosives_permit_guid']
        explosives_permit_amendment = ExplosivesPermitAmendment.query.unbound_unsafe().get(explosives_permit_amendment_guid)
        template_data = token_data['template_data']

        template_data['document_name_start_extra'] = explosives_permit_amendment.application_number
        # TODO: Remove Logs for generate document
        current_app.logger.debug(
            f'token:{token}, document_type_code: {document_type_code}, explosives_permit_amendment_document_type: {explosives_permit_amendment_document_type}, explosives_permit_amendment_guid: {explosives_permit_amendment_guid}')
        docgen_resp = DocumentGeneratorService.generate_document(
            explosives_permit_amendment_document_type.document_template, template_data)
        ...
        if not is_preview:
            ...
            document_manager_guid = DocumentManagerService.pushFileToDocumentManager(
                ...
                filename=filename,
                mine=explosives_permit_amendment.mine,
            ...
            mine_doc = MineDocument(
                mine_guid=explosives_permit_amendment.mine_guid,
            ...
            explosives_permit_amendment_document_type.after_template_generated(template_data, doc,
                                                                     explosives_permit_amendment)

        # Depending on the return_record param, return the document record or file content
        if return_record and not is_preview:
            return marshal(doc, EXPLOSIVES_PERMIT_AMENDMENT_DOCUMENT_MODEL)
