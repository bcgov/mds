...
from app.api.now_applications.now_template_transformer import (
    ...
    build_permit_package_file_label_map,
)
...
def export_permit_conditions(permit_amendment_guid, csv_writer=None):
    ...
    if is_now and amendment.now_application_identity:
        ...
        condition_variables = transform_variables_to_data(now_application, amendment, mine, total_liability)
    ...
    for condition in conditions:
        ...
        replace_variables(condition, is_now, condition_variables, label_map)

        if should_merge_with_parent(condition):
            parent = condition.parent_permit_condition
            replace_variables(parent, is_now, condition_variables, label_map)
...
def replace_variables(condition, is_now, condition_variables, label_map=None):
    if is_now:
        condition.condition = replace_condition_value_with_data(condition.condition, condition_variables, label_map)
