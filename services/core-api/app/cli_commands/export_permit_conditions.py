...
def export_permit_conditions(permit_amendment_guid, csv_writer=None):
    ...
    for condition in conditions:
        ...
        replace_variables(condition, is_now, condition_variables, now_application)

        if should_merge_with_parent(condition):
            parent = condition.parent_permit_condition
            replace_variables(parent, is_now, condition_variables, now_application)
...
def replace_variables(condition, is_now, condition_variables, now_application=None):
    if is_now:
        condition.condition = replace_condition_value_with_data(condition.condition, condition_variables, now_application)
