...
export const fetchPermitConditionsData =
  (mineGuid: string, permitGuid: string, permitAmendmentGuid: string): AppThunk<Promise<void>> =>
    (dispatch) => {
      dispatch(request(NetworkReducerTypes.GET_PERMIT_CONDITIONS_DATA));
      dispatch(showLoading());
      return CustomAxios({ errorToastMessage: String.ERROR })
        .get(
          `${ENVIRONMENT.apiUrl}${API.PERMIT_CONDITIONS_DATA(mineGuid, permitGuid, permitAmendmentGuid)}`,
          createRequestHeader()
        )
        .then((response) => {
