...
export const getCurrentComplianceCodes = createSelector(
  ...
  codes =>
    codes.filter(
      code =>
        code.expiry_date === null || new Date(code.expiry_date) > new Date()
    )
);
...
export const getDropdownIncidentStatusCodeOptions = createSelector(
  [getIncidentStatusCodeOptions],
  options =>
    createDropDownList(options, "description", "mine_incident_status_code")
);

export const getDropdownIncidentCategoryCodeOptions = createSelector(
  [getIncidentCategoryCodeOptions],
  options =>
    createDropDownList(options, "description", "mine_incident_category_code")
);
...
export const getDropdownHSRCMComplianceCodes = createSelector(
  ...
  codes =>
    codes
      .filter(
        code => code.article_act_code === "HSRCM" && code.sub_paragraph === null
      )
      .map(code => {
...
const transformMineStatusSubReason = reasons =>
  chain(reasons)
    .groupBy(
      s =>
        s.mine_operation_status_sub_reason.mine_operation_status_sub_reason_code
    )
    .filter(
      g =>
        g[0].mine_operation_status_sub_reason
          .mine_operation_status_sub_reason_code
    )
    .map(subReasons => ({
      value:
        subReasons[0].mine_operation_status_sub_reason
          .mine_operation_status_sub_reason_code,
...
const transformMineStatusReason = codes =>
  chain(codes)
    .groupBy(
      s => s.mine_operation_status_reason.mine_operation_status_reason_code
    )
    .filter(
      g => g[0].mine_operation_status_reason.mine_operation_status_reason_code
    )
    .map(reasons => ({
      value:
        reasons[0].mine_operation_status_reason
          .mine_operation_status_reason_code,
      label: reasons[0].mine_operation_status_reason.description,
      title: reasons[0].mine_operation_status_sub_reason
        .mine_operation_status_sub_reason_code
        ? null
...
const transformMineStatus = data =>
  chain(data)
    ...
    .map(codes => ({
      ...
      title: codes[0].mine_operation_status_reason
        .mine_operation_status_reason_code
        ? null
        : codes[0].description,
...
export const getDropdownMineReportDefinitionOptions = createSelector(
  [getMineReportDefinitionOptions],
  options =>
    createDropDownList(options, "report_name", "mine_report_definition_guid")
);
...
export const getDropdownNoticeOfWorkApplicationTypeOptions = createSelector(
  [getNoticeOfWorkApplicationTypeOptions],
  options =>
    createDropDownList(options, "description", "notice_of_work_type_code")
);
...
export const getDropdownNoticeOfWorkApplicationStatusOptions = createSelector(
  [getNoticeOfWorkApplicationStatusOptions],
  options =>
    createDropDownList(options, "description", "now_application_status_code")
);
