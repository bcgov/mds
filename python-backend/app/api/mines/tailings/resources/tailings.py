...
class MineTailingsStorageFacilityResource(Resource, UserMixin, ErrorMixin):
    ...
    parser.add_argument(
        'mine_guid', type=str, help='mine to create a new tsf on')
    parser.add_argument(
        'tsf_name', type=str, help='Name of the tailings storage facility.')
        ...
        if mine_tailings_storage_facility_guid:
            ...
            if not tsf:
                return self.create_error_payload(
                    404, 'mine_tailings_storage_facility not found')
            return tsf.json()
        else:
            mine_guid = request.args.get('mine_guid', type=str)
            mine_tsf_list = MineTailingsStorageFacility.find_by_mine_guid(
                mine_guid)
            if mine_tsf_list:
                return {
                    'mine_storage_tailings_facilities':
                    list(map(lambda x: x.json(), mine_tsf_list))
                }
            else:
                return self.create_error_payload(
                    404, 'Mine_guid or tsf_guid must be provided')
    ...
    def post(self, mine_tailings_storage_facility_guid=None):
        if not mine_tailings_storage_facility_guid:
            ...
            if is_mine_first_tsf:
                try:
                    documents_url = current_app.config['DOCUMENT_MS_URL'] + \
                        current_app.config['BASE_PATH'] + '/documents'

                    get_tsf_docs_resp = requests.get(
                        documents_url + '/required?category=MINE_TAILINGS',
                        headers=request.headers)
                    if get_tsf_docs_resp.status_code != 200:
                        return self.create_error_payload(
                            500, 'get_tsf_req_docs returned error' + str(
                                get_tsf_docs_resp.status_code)), 500
                    ...
                    doc_assignment_response = requests.post(
                        documents_url + '/expected/mines/' + str(mine_guid),
                        headers=request.headers,
                        json={'documents': new_expected_documents})
                    if doc_assignment_response.status_code != 200:
                        return self.create_error_payload(
                            500, 'exp_doc_assignment_post returned error' +
                            str(doc_assignment_response.status_code)), 500

                except Exception as e:
                    return self.create_error_payload(
                        500, 'Error: {}'.format(e)), 500
            return {
                'mine_tailings_storage_facilities':
                list(
                    map(
                        lambda x: x.json(),
                        MineTailingsStorageFacility.find_by_mine_guid(
                            mine_guid)))
            }
        else:
