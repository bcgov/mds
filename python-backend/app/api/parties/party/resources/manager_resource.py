...
class ManagerResource(Resource, UserMixin, ErrorMixin):
    ...
    parser.add_argument(
        ...
        help='Effective date. In the format of YYYY-MM-DD.')
    ...
    def post(self, mgr_appointment_guid=None):
        ...
        if not new_party:
            self.raise_error(
                400, 'Error: Party with guid: {}, does not exist.'.format(data['party_guid']))
        if not mine:
            self.raise_error(400,
                             'Error: Mine with guid: {}, does not exist.'.format(data['mine_guid']))
        ...
        if mine.mgr_appointment:
            ...
            if previous_mgr_expiry_date < previous_mgr.effective_date:
                self.raise_error(
                    400,
                    "Error: New manager's start date is on or before the previous manager's start date."
                )
