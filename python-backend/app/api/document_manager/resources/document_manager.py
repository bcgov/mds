...
class DocumentManagerResource(Resource, UserMixin, ErrorMixin):
    ...
    def redis_key_file_size(self, id):
        return f'document-manager/{id}/file-size'

    def redis_key_offset(self, id):
        return f'document-manager/{id}/offset'

    def redis_key_file_path(self, id):
        return f'document-manager/{id}/file-path'
    ...
    def post(self):
        ...
        cache.set(self.redis_key_file_path(document_guid), file_path, TIMEOUT_24_HOURS)
    ...
    def patch(self, document_guid=None):
        ...
        if new_offset == file_size:
            ...
            cache.set(self.redis_key_offset(document_guid), new_offset, TIMEOUT_24_HOURS)
    ...
    def head(self, document_guid):
        ...
        response.headers['Tus-Resumable'] = TUS_API_VERSION
        response.headers['Tus-Version'] = TUS_API_SUPPORTED_VERSIONS
        response.headers['Upload-Offset'] = cache.get(self.redis_key_offset(document_guid))
        response.headers['Upload-Length'] = cache.get(self.redis_key_file_size(document_guid))
