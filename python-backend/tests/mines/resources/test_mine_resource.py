...
def test_post_mine_name_exceed_chars(test_client, auth_headers):
    test_mine_data = {
        "mine_name": "012345678911234567892123456789312345678941234567895123456789512345678961",
...
def test_put_mine_tenure_mine_not_found(test_client, auth_headers):
    ...
    put_resp = test_client.put(
        '/mines/' + 'NOT_FOUND', json=test_tenure_data, headers=auth_headers['full_auth_header'])
...
def test_put_mine_tenure_invalid_length(test_client, auth_headers):
    ...
    put_resp = test_client.put(
        '/mines/' + TEST_MINE_NO, json=test_tenure_data, headers=auth_headers['full_auth_header'])
    put_data = json.loads(put_resp.data.decode())
...
def test_put_mine_tenure_guid(test_client, auth_headers):
    ...
    put_resp = test_client.put(
        '/mines/' + TEST_MINE_GUID, json=test_tenure_data, headers=auth_headers['full_auth_header'])
    put_data = json.loads(put_resp.data.decode())
    assert put_resp.status_code == 200
    assert test_tenure_data['tenure_number_id'] in [
        x['tenure_number_id'] for x in put_data['mineral_tenure_xref']
...
def test_put_mine_major_true(test_client, auth_headers):
    ...
    put_resp = test_client.put(
        '/mines/' + TEST_MINE_GUID, json=test_mine_data, headers=auth_headers['full_auth_header'])
    put_data = json.loads(put_resp.data.decode())
...
def test_put_mine_region(test_client, auth_headers):
    test_mine_data = {"mine_region": TEST_REGION_CODE, "mine_note": ""}
    put_resp = test_client.put(
        '/mines/' + TEST_MINE_GUID, json=test_mine_data, headers=auth_headers['full_auth_header'])
    assert put_resp.status_code == 200
    put_data = json.loads(put_resp.data.decode())
