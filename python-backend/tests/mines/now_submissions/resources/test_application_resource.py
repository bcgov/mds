...
class TestGetMineApplicationResource:
    ...
    def test_get_mine_application_list_filter_by_trackingnumber(self, test_client, db_session, auth_headers):
        ...
        mine = MineFactory(minimal=True)
        now_submission_1 = NOWSubmissionFactory(mine=mine, trackingnumber=1)
        identity_1 = NOWApplicationIdentityFactory(now_submission=now_submission_1, mine=mine)
        now_submission_2 = NOWSubmissionFactory(mine=mine, trackingnumber=1)
        identity_2 = NOWApplicationIdentityFactory(now_submission=now_submission_2, mine=mine)
        now_submission_3 = NOWSubmissionFactory(mine=mine, trackingnumber=12)
        identity_3 = NOWApplicationIdentityFactory(now_submission=now_submission_3, mine=mine)
        now_submission_4 = NOWSubmissionFactory(mine=mine, trackingnumber=10305)
        identity_4 = NOWApplicationIdentityFactory(now_submission=now_submission_4, mine=mine)

        get_resp = test_client.get(f'mines/{mine.mine_guid}/now-applications?tracking_number=1',
                                   headers=auth_headers['full_auth_header'])
        get_data = json.loads(get_resp.data.decode())
    ...
    def test_get_mine_application_list_filter_by_multiple_filters(self, test_client, db_session, auth_headers):
        ...
        get_resp = test_client.get(
            f'mines/{mine.mine_guid}/now-applications?status=app,rej&notice_of_work_type_search=dog&tracking_number=1',
            headers=auth_headers['full_auth_header'])
        get_data = json.loads(get_resp.data.decode())
