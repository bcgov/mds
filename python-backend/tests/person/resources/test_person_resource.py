...
def test_get_person_not_found(test_client, auth_headers):
    get_resp = test_client.get('/party/' + TEST_MINE_GUID, headers=auth_headers['full_auth_header'])
...
def test_post_person_invalid_url(test_client, auth_headers):
    ...
    post_resp = test_client.post('/party/some_id', data=test_person_data, headers=auth_headers['full_auth_header'])
