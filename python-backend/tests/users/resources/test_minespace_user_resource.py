...
def test_get_minespace_users_all(test_client, db_session, auth_headers):
    ...
    assert get_data['users'][0]["email"] == user_email
...
def test_get_minespace_user_by_email(test_client, db_session, auth_headers):
    ...
    get_resp = test_client.get(
        f'/users/minespace?email={user.email}', headers=auth_headers['full_auth_header'])
    get_data = json.loads(get_resp.data.decode())
    assert get_resp.status_code == 200, get_resp.response
    assert get_data["email"] == user.email
...
def test_delete_minespace_missing_id(test_client, db_session, auth_headers):
