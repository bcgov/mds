...
def test_put_new_file(test_client, db_session, auth_headers):
    ...
    put_resp = test_client.put(
        f'/permits/{permit_amendment.permit.permit_guid}/amendments/{permit_amendment.permit_amendment_guid}/documents',
...
def test_happy_path_file_removal(test_client, db_session, auth_headers):
    ...
    del_resp = test_client.delete(
        f'/permits/{doc.permit_amendment.permit.permit_guid}/amendments/{doc.permit_amendment.permit_amendment_guid}/documents/{doc.permit_amendment_document_guid}',
        headers=auth_headers['full_auth_header'])
...
def test_remove_file_no_doc_guid(test_client, db_session, auth_headers):
    ...
    del_resp = test_client.delete(
        f'/permits/{doc.permit_amendment.permit.permit_guid}/amendments/{doc.permit_amendment.permit_amendment_guid}/documents',
        headers=auth_headers['full_auth_header'])
...
def test_remove_file_no_doc(test_client, db_session, auth_headers):
    ...
    del_resp = test_client.delete(
        f'/permits/{uuid.uuid4()}/amendments/{doc.permit_amendment.permit_amendment_guid}/documents/{uuid.uuid4()}',
        headers=auth_headers['full_auth_header'])
    post_data = json.loads(del_resp.data.decode())
...
def test_remove_file_no_exp_doc(test_client, db_session, auth_headers):
    ...
    del_resp = test_client.delete(
        f'/permits/{uuid.uuid4()}/amendments/{uuid.uuid4()}/documents/{doc.permit_amendment_document_guid}',
        headers=auth_headers['full_auth_header'])
    post_data = json.loads(del_resp.data.decode())
    assert del_resp.status_code == 404
