...
def test_happy_path_file_upload(test_client, auth_headers, setup_info):
    ...
    filename = list(post_data['document_manager_guids'].keys())[0] + '.pdf'
    saved_file = open(
        f'{test_dir_path}/mines/{setup_info.get("mine_guid")}/tailings/{filename}'
    )

    assert saved_file


def test_happy_path_file_upload_two_files(test_client, auth_headers,
                                          setup_info):
    test_dir = os.mkdir(setup_info.get('base_path') + '/test')
    test_dir_path = setup_info.get('base_path') + '/test'
    ...
    data = {
        'folder':
        f'test/mines/{setup_info.get("mine_guid")}/tailings',
        'file':
        [setup_info.get('file_upload_2'),
         setup_info.get('file_upload_1')],
        'pretty_folder':
        f'test/mines/{setup_info.get("mine_no")}/tailings',
    }
    ...
    post_data = json.loads(post_resp.data.decode())
    filename = list(post_data['document_manager_guids'].keys())[0] + '.docx'
    filename2 = list(post_data['document_manager_guids'].keys())[1] + '.pdf'
    saved_file = open(
        f'{test_dir_path}/mines/{setup_info.get("mine_guid")}/tailings/{filename}'
    )
    saved_file_2 = open(
        f'{test_dir_path}/mines/{setup_info.get("mine_guid")}/tailings/{filename2}'
    )
...
def test_bad_file_type(test_client, auth_headers, setup_info):
    ...
    data = {
        ...
        'file': [
            setup_info.get('file_upload_2'),
            setup_info.get('file_upload_3'),
            setup_info.get('file_upload_4')
        ],
        'pretty_folder':
        f'test/mines/{setup_info.get("mine_no")}/tailings',
    }
...
def test_download_file(test_client, auth_headers, setup_info):
    ...
    get_resp = test_client.get(
        f'/document-manager/{list(post_data["document_manager_guids"].keys())[0]}',
        headers=auth_headers['full_auth_header'])
    saved_file.seek(0)
