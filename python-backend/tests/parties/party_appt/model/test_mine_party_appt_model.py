...
def test_party_appt_model_find_by_party_guid(test_client, auth_headers):
    mine_party_appts = MinePartyAppointment.find_by_party_guid(TEST_PARTY_PER_GUID_1)
    assert all(mpa.party_guid == TEST_PARTY_PER_GUID_1 for mpa in mine_party_appts)

def test_party_appt_model_find_by_mine_guid(test_client, auth_headers):
    mine_party_appts = MinePartyAppointment.find_by_mine_guid(TEST_MINE_GUID)
    assert all(mpa.mine_guid == TEST_MINE_GUID for mpa in mine_party_appts)
