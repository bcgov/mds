...
def test_get_required_document_by_guid(test_client, db_session, auth_headers):
    ...
    get_resp = test_client.get(
        '/documents/required/' + str(req_guid), headers=auth_headers['full_auth_header'])
    get_data = json.loads(get_resp.data.decode())
    assert get_resp.status_code == 200
...
def test_get_all_required_documents_by_category(test_client, db_session, auth_headers):
    ...
    get_resp = test_client.get(
        '/documents/required?category=' + cat, headers=auth_headers['full_auth_header'])
    get_data = json.loads(get_resp.data.decode())
    assert get_resp.status_code == 200
...
def test_get_all_required_documents_by_category_and_sub_category(test_client, db_session,
    ...
    get_resp = test_client.get(
        f'/documents/required?category={cat}&sub_category={sub_cat}',
        headers=auth_headers['full_auth_header'])
    get_data = json.loads(get_resp.data.decode())
    assert get_resp.status_code == 200
    assert len(get_data['required_documents']) == len(
        RequiredDocument.query.filter_by(
            req_document_category=cat, req_document_sub_category_code=sub_cat).all())
