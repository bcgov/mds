from tests.constants import TEST_MINE_GUID, TEST_EXPECTED_DOCUMENT_GUID2, TEST_EXPECTED_DOCUMENT_NAME2
...
def test_expected_documents_find_by_mine_guid(test_client, auth_headers):
    ...
    assert all(ed.mine_guid == TEST_MINE_GUID for ed in expected_documents)
...
def test_expected_documents_find_by_mine_guid(test_client, auth_headers):
    ...
    assert all(oed.mine_guid == TEST_MINE_GUID for oed in org_expected_documents)
    assert all(ned.mine_guid == TEST_MINE_GUID for ned in new_expected_documents)