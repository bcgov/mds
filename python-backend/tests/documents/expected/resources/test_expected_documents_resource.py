...
def test_get_expected_document_by_guid(test_client, auth_headers):
    get_resp = test_client.get('/documents/expected/' + TEST_EXPECTED_DOCUMENT_GUID1 , headers=auth_headers['full_auth_header'])
...
def test_put_expected_document_by_guid(test_client, auth_headers):
    ...
    put_resp = test_client.put('/documents/expected/' + TEST_EXPECTED_DOCUMENT_GUID1 , json={'document':put_exp_doc}, headers=auth_headers['full_auth_header'])
...
def test_del_expected_document_by_guid(test_client, auth_headers):
    del_resp = test_client.delete('/documents/expected/' + TEST_EXPECTED_DOCUMENT_GUID1 , headers=auth_headers['full_auth_header'])
