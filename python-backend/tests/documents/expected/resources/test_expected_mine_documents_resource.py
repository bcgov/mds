...
def test_get_all_expected_documents_by_mine_guid(test_client, auth_headers):
    get_resp = test_client.get('/documents/expected/mines/' + TEST_MINE_GUID , headers=auth_headers['full_auth_header'])
    get_data = json.loads(get_resp.data.decode())
    assert get_resp.status_code == 200
    assert len(get_data['expected_mine_documents']) == 1
...
def test_post_expected_documents_by_mine_guid(test_client, auth_headers):
    ...
    post_resp = test_client.post('/documents/expected/mines/' + TEST_MINE_GUID , json=post_documents, headers=auth_headers['full_auth_header'])
    post_data = json.loads(post_resp.data.decode())
    assert post_resp.status_code == 200
    assert len(post_data['expected_mine_documents']) == 1
    assert post_data['expected_mine_documents'][0]['req_document_guid'] == TEST_REQUIRED_REPORT_GUID2
...
def test_get_all_expected_documents_by_mine_guid_after_insert(test_client, auth_headers):
    ...
    assert all(ed['mine_guid'] == TEST_MINE_GUID for ed in get_data['expected_mine_documents'])
