...
def test_get_mine_no_auth(test_client):
    get_resp = test_client.get('/mines/location/' + TEST_LOCATION_GUID, headers={})
    assert get_resp.status_code == 401


def test_get_mine_view_only(test_client, auth_headers):
    get_resp = test_client.get('/mines/location/' + TEST_LOCATION_GUID, headers=auth_headers['view_only_auth_header'])
    assert get_resp.status_code == 200
